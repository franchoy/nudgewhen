...
Historical v0.1.0, v0.1.1, v0.1.2, and v0.1.3 release documents remain under `docs/releases/` for traceability and are not normative for active v0.1.4 work.
...
- **Release title:** `NudgeWhen v0.1.4 — Mark Reminder Done`
...
- **Phase 3 implementation:** `LANDED_AND_EXACT_HEAD_CI_ACCEPTED`
- **Phase 4 implementation:** `LANDED_AND_EXACT_HEAD_CI_ACCEPTED`
- **Release readiness:** `NO`
- **Merged / tagged / published:** `NO / NO / NO`
- **Product scope:** `["reminders", "persistence"]` (unchanged from v0.1.3; done-state behavior belongs to the existing `reminders` capability; persistent completion state belongs to the existing `persistence` capability)
- **Persistence-format decision:** `NWR2_WITH_NWR1_BACKWARD_LOAD` (Phase 1 selected NWR2 writes with backward NWR1 loading and lazy same-file migration; after the Phase-3 implementation landing, the active v0.1.4 release-branch `FileReminderStore` candidate uses NWR2 writes with NWR1 backward loading and lazy same-file migration on the first successful persistence-changing save, with no load-time rewrite and order / id / text / done-state preservation under the frozen Phase-3 contract; the latest stable published v0.1.3 release remains NWR1)
- **Current committed Android identity:** `versionCode 5 / versionName 0.1.4` (Phase 5 identity landing landed Android identity to the v0.1.4 target)
- **Latest stable published release:** `v0.1.3`
...
- **Pre-v0.1.4 maintenance window (closed):** PR #11 setup-java v6 was merged into `main` before the `release/v0.1.4` branch was created; the current `setup-java` pin is `v6.0.0`; AGP remains `9.2.1`; Gradle remains `9.4.1`; PR #13 / AGP 9.4 migration is outside the v0.1.4 release train and is deferred until after v0.1.4. This maintenance-window closure does not waive or alter any exact-path, baseline, hard-stop, privacy, network, staging/commit/push, dependency/configuration, or Category-C rule.
- **v0.1.4 Phase 0 closure record:** Phase 0 — Release Definition & Bootstrap — is `Complete` for the active v0.1.4 release on `release/v0.1.4`. Phase 0 performed the v0.1.4 release-definition, governance, and document-bootstrap synchronization, including the active release charter (`docs/releases/v0.1.4/release-charter.md`), phase list (`docs/releases/v0.1.4/phase-list.md`), current-facing tracked governance, machine-readable release contract (`scripts/release_contract.json`), local-validation documentation, and the initial dirty-candidate repository-consistency validation. Phase 0 did not implement any done-state functionality; did not modify `app/build.gradle.kts`; did not modify any product Kotlin; did not modify persistence production code; did not modify Compose; did not perform Android identity alignment; and did not select a concrete persistence representation. The committed Android artifact identity remained unchanged at `versionCode 4 / versionName 0.1.3`; the v0.1.4 target identity of `versionCode 5 / versionName 0.1.4` is recorded as a future target only. Phase 1 — Done State Architecture Contract — is the next lifecycle phase, is `Planned`, and has not started. The product-scope authorization remains exactly `["reminders", "persistence"]`. The persistence-format decision is `PHASE_1_REQUIRED`. v0.1.4 is **not** merged, **not** tagged, **not** published, and is not claimed release-ready.
- **v0.1.4 Phase 1 closure record:** Phase 1 — Done State Architecture Contract — is `Complete` for the active v0.1.4 release on `release/v0.1.4`. Phase 1 architecture lifecycle target: `Complete`. Architecture path: `docs/releases/v0.1.4/done-state-architecture.md`. Architecture commit: `84446af3d6360cbcb35371fbf451ad9d1b180691`. Parent: `193fe336b2b147f897b87636b29a2d439d2e1219`. Exact-head CI run: `34686932221` / `success`. Validate job: `103535431436` / `success`. Architecture repository boundary: `LANDED_AND_EXACT_HEAD_CI_ACCEPTED`. Phase 1 was architecture-only; no product implementation occurred in Phase 1. Phase 2 — Done State Domain Implementation & JVM Proof — is the next lifecycle phase, is `Planned`, and has not started. The product-scope authorization remains exactly `["reminders", "persistence"]`. The persistence-format decision is `NWR2_WITH_NWR1_BACKWARD_LOAD`; current production/released persistence remains NWR1; Phase 3 owns implementation, backward-load behavior, lazy same-file migration, and persistence proof. v0.1.4 is **not** merged, **not** tagged, **not** published, and is not claimed release-ready.
- **v0.1.4 Phase 2 closure record:** Phase 2 — Done State Domain Implementation & JVM Proof — is `Complete` for the active v0.1.4 release on `release/v0.1.4`. Implementation commit: `658f607f4fda3e886fecdd7e785d325b37a31010`. Parent: `0d93dea8446a45d76c3a8c869fdc02e8b2944e32`. Subject: `feat: implement v0.1.4 done-state domain`. Repository boundary: `LANDED_AND_EXACT_HEAD_CI_ACCEPTED`. Implementation exact-head CI: `34751025153` / `success`. Validate job: `103707474220` / `success`. Principal implementation proof: `ReminderTest`: `2 / 0 / 0 / 0`; `ReminderControllerTest`: `51 / 0 / 0 / 0`; affected-class total `53`; standard regression `244 / OK`; `required` `11 / 0 / 0`; `docs` `11 / 0 / 0`; `skip-Android` `22 / 0 / 0`. Implementation evidence: `EXP-0052`. Closure-sync planning + Build evidence: `EXP-0053`. Phase 2 added the deterministic `Reminder.done` default-`false` state, the deterministic `ReminderController.setDone(id, done)` API, the deterministic `edit()` preservation of done state, and the deterministic JVM proof for the affected domain classes; Phase 2 did not modify persistence production, did not modify Compose, did not perform Android identity alignment, did not add a Gradle or test dependency, did not change validator architecture, and did not change CI workflow. Phase 3 — Persistence Compatibility / Migration Proof — has since completed and is `Complete`; Phase 4 — Minimal Compose Integration — has since completed and is `Complete`; Phase 5 — Integration & Device Validation — has since completed and is `Complete`; Phase 6 — Integrated Audit & Reconciliation — is the next lifecycle phase and is `Planned` / `NOT_STARTED` / `NOT_AUTHORIZED`. The product-scope authorization remains exactly `["reminders", "persistence"]`. The persistence-format decision remains `NWR2_WITH_NWR1_BACKWARD_LOAD`; the active v0.1.4 release-branch `FileReminderStore` candidate uses NWR2 writes with NWR1 backward loading and lazy same-file migration on the first successful persistence-changing save, with no load-time rewrite and order / id / text / done-state preservation under the frozen Phase-3 contract; the latest stable published v0.1.3 release remains NWR1. v0.1.4 is **not** merged, **not** tagged, **not** published, and is not claimed release-ready.
- **v0.1.4 Phase 3 closure record:** Phase 3 — Persistence Compatibility / Migration Proof — is `Complete` for the active v0.1.4 release on `release/v0.1.4`. Implementation commit: `6c3ca643d47acf94e813d4f62f904d43b967a6e1`. Parent: `fe876b86977f0f34bded3c3e774ffcaec016591f`. Subject: `feat: implement v0.1.4 persistence migration`. Repository boundary: `LANDED_AND_EXACT_HEAD_CI_ACCEPTED`. Implementation exact-head CI: `CI / 6c3ca643d47acf94e813d4f62f904d43b967a6e1 / release/v0.1.4 / push / success`. Implementation evidence: `EXP-0056`. Phase 3 implemented the frozen Phase-1-selected `NWR2_WITH_NWR1_BACKWARD_LOAD` representation on the active v0.1.4 release-branch `FileReminderStore`: NWR2 writes; backward NWR1 loading; lazy same-file migration on the first successful persistence-changing save; no load-time rewrite; order / id / text / done-state preservation under the frozen Phase-3 contract. Phase 3 added the deterministic JVM proof for persistence across done-state toggling, restart survival of done state, and backward loading of existing v0.1.3 NWR1 data. Phase 3 did not modify Compose, did not perform Android identity alignment, did not add a Gradle or test dependency, did not change validator architecture, and did not change CI workflow. Phase 4 — Minimal Compose Integration — has since completed and is `Complete`; Phase 5 — Integration & Device Validation — has since completed and is `Complete`; Phase 6 — Integrated Audit & Reconciliation — is the next lifecycle phase and is `Planned` / `NOT_STARTED` / `NOT_AUTHORIZED`. The product-scope authorization remains exactly `["reminders", "persistence"]`. The persistence-format decision remains `NWR2_WITH_NWR1_BACKWARD_LOAD`; the latest stable published v0.1.3 release remains NWR1. v0.1.4 is **not** merged, **not** tagged, **not** published, and is not claimed release-ready.
- **v0.1.4 Phase 4 closure record:** Phase 4 — Minimal Compose Integration — is `Complete` for the active v0.1.4 release on `release/v0.1.4`. Implementation commit: `e173dd69335e0b1dcbade07b4797c78f91c0668e`. Parent: `d15703c8667fd228475b34a018ade8177a2be67c`. Subject: `feat: implement v0.1.4 mark reminder done UI`. Repository boundary: `LANDED_AND_EXACT_HEAD_CI_ACCEPTED`. Implementation evidence: `EXP-0058`. Frozen production SHA: `ReminderScreen.kt`: `b8e84da641d6ecab9727a9477a1fc265ddc2e5b8eea416c83bcb8cd3f8965f06`. Frozen EXP SHA: `EXP-0058`: `6da6409c077e2f843e276921790e27be73dec0a3d717ca4cd8329ce0cbf8f2ec`. Implementation exact-head CI: `CI / e173dd69335e0b1dcbade07b4797c78f91c0668e / release/v0.1.4 / push / success`. Phase 4 implemented the bounded source-level done-state Compose integration on the existing `ReminderScreen`: a controlled Material3 Checkbox is integrated into both normal and editing reminder rows; checked state is sourced from `reminder.done`; action delegates to `controller.setDone(reminder.id, newChecked)`; screen refresh occurs only after accepted success; no second done-state UI authority exists; no optimistic done-state publish exists; editing state is preserved during done toggling; no done-specific sort, reorder, removal, dimming, or text decoration was added. Phase 4 did not perform Android identity alignment, did not perform integrated device validation, did not introduce a `ViewModel`, `Flow`, coroutine, DI, or navigation solely for done state, did not add a new Android `Activity`, `Service`, `Receiver`, or `Provider`, did not add a new Android permission, did not introduce a Gradle dependency or test dependency, did not change validator architecture, and did not change CI workflow. Phase 5 — Integration & Device Validation — has since completed and is `Complete`; Phase 6 — Integrated Audit & Reconciliation — is the next lifecycle phase and is `Planned` / `NOT_STARTED` / `NOT_AUTHORIZED`. The product-scope authorization remains exactly `["reminders", "persistence"]`. The persistence-format decision remains `NWR2_WITH_NWR1_BACKWARD_LOAD`; the latest stable published v0.1.3 release remains NWR1. The committed Android artifact identity is now `versionCode 5 / versionName 0.1.4`; Phase-5 identity landing landed Android identity to the v0.1.4 target. v0.1.4 is **not** merged, **not** tagged, **not** published, and is not claimed release-ready.
- **v0.1.4 Phase 5 closure record:** Phase 5 — Integration & Device Validation — is `Complete` for the active v0.1.4 release on `release/v0.1.4`. Identity landing commit: `e8c9c08920f5a0c146189c0087b293c260c2701d`. Parent (Phase-4 closure commit): `846d291d2e262fbad7c1e03d2fa1e3b7ec63cf10`. Subject: `chore: align v0.1.4 android identity`. Implementation repository boundary: `LANDED_AND_EXACT_HEAD_CI_ACCEPTED`. Android identity: `versionCode 5 / versionName 0.1.4`. Phase-5 cumulative evidence: `EXP-0060`. P5_01 through P5_13: `ACCEPTED`. P5_13: `ONE_PHYSICAL_DEVICE_ONLY`. `GENERAL_ANDROID_COMPATIBILITY`: `NOT_CLAIMED`. `MULTI_DEVICE_VALIDATION`: `NOT_CLAIMED`. `PRODUCTION_READINESS`: `NOT_CLAIMED`. `SUPPORTED_ANDROID_VERSION_RANGE`: `NOT_CLAIMED`. Phase 5 advanced the Android artifact identity from `versionCode 4 / versionName 0.1.3` to the v0.1.4 target `versionCode 5 / versionName 0.1.4`; Phase 5 did not add a new Android permission, did not add a new Android component, did not introduce a new Gradle dependency, did not introduce a new test dependency, did not change validator architecture, did not make a general Android compatibility claim, and did not make a production-readiness claim. Next lifecycle phase: `Phase 6 — Integrated Audit & Reconciliation`; Phase 6: `Planned` / `NOT_STARTED` / `NOT_AUTHORIZED`. Phase-5-G closure synchronization commit: `e5e88c8044e819ffd6b45fdde5d7cc342575ae0f`. Parent: `e8c9c08920f5a0c146189c0087b293c260c2701d`. Subject: `docs: close v0.1.4 phase 5`. Repository boundary: `LANDED_AND_EXACT_HEAD_CI_ACCEPTED`. Remote post-Push SHA: `e5e88c8044e819ffd6b45fdde5d7cc342575ae0f`. Exact hosted CI: `CI / e5e88c8044e819ffd6b45fdde5d7cc342575ae0f / release/v0.1.4 / push / success`. Native closure transaction: `LANDED / S10_LANDED`. Terminal maintainer acceptance: `PHASE_5_FORMALLY_COMPLETE = YES`. v0.1.4 is **not** merged, **not** tagged, **not** published, and is not claimed release-ready.
- **v0.1.4 Phase 6 closure record (candidate):** Phase 6 — Integrated Audit & Reconciliation — is `Complete` for the active v0.1.4 release on `release/v0.1.4`. Phase 6 owns the integrated audit and reconciliation pass for v0.1.4. Phase 6 evidence: `EXP-0061`. Phase-6-A retained validation: standard Python regression `244 / OK`; `required` group `11 / 0 / 0`; `docs` group `11 / 0 / 0`; `skip-Android` group `22 / 0 / 0`; `release_gate` for those partial validators: `NOT_SATISFIED`. The retained Phase-6-A dirty candidate was accepted for formal closure synchronization by `PHASE_6_B_INDEPENDENT_AUDIT_RECOVERY_R2: PASS_CORRECTED_PHASE6A_TWO_PATH_CANDIDATE_ACCEPTED_FOR_PHASE6C`. Phase 6 introduced no product functionality, no Android behavior change, no Android permission change, no Android component change, no Gradle dependency change, no test dependency change, no validator-architecture change, and no CI workflow change. Phase 6 product/device scope remains unchanged: `["reminders", "persistence"]`; `DEVICE_PROOF: ONE_PHYSICAL_DEVICE_ONLY`; `GENERAL_ANDROID_COMPATIBILITY: NOT_CLAIMED`; `MULTI_DEVICE_VALIDATION: NOT_CLAIMED`; `SUPPORTED_ANDROID_VERSION_RANGE: NOT_CLAIMED`; `PRODUCTION_READINESS: NOT_CLAIMED`. Next lifecycle phase: `Phase 7 — Full Pre-Release Gate`; Phase 7: `Planned` / `NOT_STARTED` / `NOT_AUTHORIZED`. This Phase 6 candidate closure record does NOT claim: a Phase-6 closure Commit SHA; Phase-6 push; Phase-6 exact-head CI; native `S10_LANDED`; terminal Phase-6 maintainer acceptance; or `release_gate=SATISFIED`. The full 39-pass all-groups release gate belongs to Phase 7 and is NOT claimed by Phase-6-C. v0.1.4 is **not** merged, **not** tagged, **not** published, and is not claimed release-ready.
- **v0.1.4 Phase 7 closure record (candidate):** Phase 7 — Full Pre-Release Gate — is `Complete` for the active v0.1.4 release on `release/v0.1.4` as a closure candidate. Phase-7 evidence: `EXP-0062`. Definitive pre-release validation HEAD: `13454cd45994fa041196f87137d3cbcb77be065a`. Standard regression: `244 / OK`. Ordinary all-groups inventory: `39 / 0 / 0`. Clean full-gate inventory: `41 / 0 / 0`. Clean full-gate result: `release_gate=SATISFIED`. Android identity: `versionCode 5 / versionName 0.1.4`. Device proof: `ONE_PHYSICAL_DEVICE_ONLY`. `GENERAL_ANDROID_COMPATIBILITY`: `NOT_CLAIMED`. `MULTI_DEVICE_VALIDATION`: `NOT_CLAIMED`. `SUPPORTED_ANDROID_VERSION_RANGE`: `NOT_CLAIMED`. `PRODUCTION_READINESS`: `NOT_CLAIMED`. Candidate lifecycle projection: `8 Complete / 0 Planned`. This Phase 7 candidate closure record does NOT yet claim: a Phase-7 closure Commit SHA; Phase-7 push; Phase-7 remote SHA; Phase-7 exact-head CI; native `S10_LANDED`; terminal Phase-7 maintainer acceptance; merge; tag; publication; or `release_readiness=YES`. Formal terminal acceptance still requires Phase-7-D independent candidate audit, Phase-7-E native nudge-land landing, and Phase-7-F terminal maintainer acceptance. v0.1.4 is **not** merged, **not** tagged, **not** published, and is not claimed release-ready.
- **v0.1.3 historical closure record (preserved):** v0.1.3 — Editable Local Reminders — remains the latest stable published release and is historical evidence for the active v0.1.4 train. The complete v0.1.3 eight-phase lifecycle and its synchronized state remain unchanged:
  - **Phase 0 — Release Definition & Bootstrap:** `Complete` (formal-closure Phase 0-C6 accepted at `1 Complete / 7 Planned`).
  - **Phase 1 — Editing Architecture Contract:** `Complete`; architecture commit `9004b0f90f60d2d5c8b1ac4828d0a4521316ae5a`; exact-head CI run `33183197545 / success`; architecture path `docs/releases/v0.1.3/editing-architecture.md` (frozen SHA-256 `71a8c56e3515a17ebfebd4bde42be90991caab7afeb8528ca4173eec3a0630c5`).
  - **Phase 2 — Editing Domain Implementation & JVM Proof:** `Complete`; implementation commit `7eacbe3746807a36fecc2a33aac8768f30287686` (parent `2fa810cce63e60c06d6c2b9ad04d80c30db2368d`); exact-head CI run `33239803189 / success`; B3 frozen SHAs production `56785070525e37530b9d8872ad2c4508d931bb58d3a8fa3b39f652bb423326ca` and test `fa9aa6029eed9ff659e724aff48403181dcc466083b15f6cc4040f74c044a1f3`; controller test total `42`.
  - **Phase 3 — Persistence Compatibility Proof:** `Complete`; implementation commit `b77af048950a720482c4ec279762d51f7f65ca5f` (parent `fa387ece5eeb972f4344ea4bd04582749b6bbf02`); exact-head CI run `33245690596 / success`; B3 frozen SHA `74e46f94fcfbc823911faf971124c4600f70f5d02002b5dc4e6f2a1d623aa4a7`; NWR1 migration `NOT_REQUIRED_BY_PHASE_3_TEST_EVIDENCE`; source-level `FileReminderStoreTest` count `39`.
  - **Phase 4 — Minimal Compose Editing UX:** `Complete`; implementation commit `673951082061562b45a40096bc2f9f5debdfb72d` (parent `938a38c7b2ba659e11eb31588b3a21526f180492`); exact-head CI run `33250408328 / success`; B3 frozen SHA `322c57bb2adfa192e0eb3115e9bfb5233bc7e29beb18a2ef9641411be9bb3add`.
  - **Phase 5 — Integration & Device Validation:** `Complete` in the v0.1.3 closure-synchronization candidate; identity commit `1cfb9c373abfa24cf10f23daa152f4a410932d26` (parent `3ba8470918a28acd4376639cf061f20209f78735`); exact-head identity CI run `33255108409 / success`; delivered CURRENT identity `versionCode 4 / versionName 0.1.3`; frozen APK SHA-256 `209bfb2a11628a903d7163ca807776a01aeb107e9fbe4c7168e6984a715f8908`; physical-device runtime evidence `ONE_PHYSICAL_DEVICE_ONLY`; general Android compatibility, multi-device validation, and production readiness `NOT_CLAIMED`.
  - **Phase 6 — Integrated Audit & Reconciliation:** `Complete` in the v0.1.3 closure-synchronization candidate; Phase 6-C real-use nudge-land evidence `Complete` at landing-core commit `844db073394b9d76e847f4078b7eb12b79b07a78` (terminal state `LANDED / S10_LANDED`); single-use Phase 6-C authorization `nudgewhen-v0.1.3-phase6c-first-use-001` `PROHIBITED` from reuse.
  - **Phase 7 — Full Pre-Release Gate:** `Complete` in the v0.1.3 closure-synchronization candidate; first Phase-7 landing commit `4213fd2f6750a71bc91d7e3516e043bf037aaff1` (terminal state `LANDED / S10_LANDED`); single-use authorization `nudgewhen-v0.1.3-phase7b-first-use-001` `PROHIBITED` from reuse; standard regression `244 / OK`; definitive clean validator `41 / 0 / 0`; `release_gate=SATISFIED`; post-validator HEAD `4213fd2f6750a71bc91d7e3516e043bf037aaff1`; final Phase-7-C local observation `CLEAN`.
  - At the retained Phase-7 pre-release evidence point, v0.1.3 had not yet been merged, tagged, or published and was not claimed release-ready. Those values are historical-at-that-time evidence. v0.1.3 has since completed its release-bearing merge, annotated tag, and GitHub Release and remains the latest stable published release. No v0.1.3 release action is authorized or repeated by the active v0.1.4 train.
...
- **Phase 2 closure record:** Phase 2 is `Complete`. The accepted Phase 2 evidence is `EXP-0037`. The Phase 2 domain-core commit is `6781bccacb5324dde854a5001a939754bb309165` (subject `feat: add reminder domain core`, parent `c1dc3a5c94cf4116cf81d4b404694e3e4bf28a7a`); the domain-core exact-head CI run is `32708073861` (workflow `CI`, event `push`, branch `release/v0.1.2`, head SHA `6781bccacb5324dde854a5001a939754bb309165`, conclusion `success`); the retained domain-core evidence is `Reminder`, `ReminderStore`, `ReminderController`, JUnit 4.13.2 as the Phase 2 test-only dependency, 2 `Reminder` tests, 25 `ReminderController` tests, 27 JVM tests total, 0 skipped, 0 failures, 0 errors, and no Android-framework ownership in the Phase 2 domain code. The validator-integration commit is `ba6a581f00ad2d5d4f550f95e6ccfa5da716825f` (subject `test: integrate reminder JVM tests into validator`, parent `6781bccacb5324dde854a5001a939754bb309165`); the validator exact-head CI run is `32711898852` (workflow `CI`, event `push`, branch `release/v0.1.2`, head SHA `ba6a581f00ad2d5d4f550f95e6ccfa5da716825f`, conclusion `success`); the retained validator evidence is the existing android group retained, no new validation group added, the android/jvm-tests check added in the order `gradle-projects -> jvm-tests -> gradle-build`, the separate Gradle task `:app:testDebugUnitTest`, JVM failure short-circuits the later build/APK/manifest checks, validator regression `100 tests / OK`, required `11/0/0`, docs `11/0/0`, android offline `17/0/0`, and `release_gate=NOT_SATISFIED`. Phase 3 — Local Persistence — is now the current lifecycle phase and remains `Planned`; Phase 3 implementation has not started.
- **v0.1.2 Phase 3 closure record:** Phase 3 is `Complete`. The designated cumulative Phase 3 evidence record is `EXP-0038`. The Phase 3 implementation commit is `c3eb1b580b744111ed3024cfbd58a8ce3113ad35` (subject `feat: add local reminder persistence`, parent `31f9e255b0b3be56c08fb6c4bd4bf13271463d2b`, branch `release/v0.1.2`); the exact-head implementation CI run is `32720528488` (conclusion `success`). Production implementation is `FileReminderStore` at `app/src/main/kotlin/io/github/franchoy/nudgewhen/data/FileReminderStore.kt`. The persistence test path is `app/src/test/kotlin/io/github/franchoy/nudgewhen/data/FileReminderStoreTest.kt` with 29 Phase 3 persistence tests `P3_01` through `P3_29`. The retained combined JVM evidence is 56 tests, 0 failures, 0 errors, 0 skipped. The retained Python validator regression accepted 100 tests / OK. The retained `required` group ran `11/0/0`, the retained `docs` group ran `11/0/0`, the retained `android offline` group ran `17/0/0`, and `release_gate=NOT_SATISFIED`. Phase 3B-R1 corrected only the false CR/UTF-8 source comment; the executable persistence behavior and tests were unchanged. The Phase 3 implementation adds local `FileReminderStore` persistence with JVM evidence that proves persistence round-trip, persisted ordering, removal persistence at the store/domain boundary, and restoration using a newly constructed `FileReminderStore`/controller. The Phase 3 implementation does not add `MainActivity` wiring to `FileReminderStore`, application-startup restoration through `MainActivity`, `ReminderScreen` implementation, or the complete user-facing reminder lifecycle. Phase 4 — Minimal Android Reminder UI — is now the current lifecycle phase and remains `Planned`; Phase 4 implementation has not started. The Phase 3 closure-synchronization commit, push, and exact-head CI are separate maintainer repository actions; this record does not claim that those actions have already occurred.
- **v0.1.2 Phase 4 closure record:** Phase 4 — Minimal Android Reminder UI — is `Complete`. Cumulative Phase 4 experiment evidence is `EXP-0039`. The Phase 4 implementation commit is `05503d58416e287afd96cc1fc7c6f78df8fd2784` (subject `feat: add minimal reminder UI`, parent `1e612d5c43c740f5aabfc4825992fce8ae8c7e9e`, branch `release/v0.1.2`). The exact implementation paths are `app/src/main/kotlin/io/github/franchoy/nudgewhen/MainActivity.kt` and `app/src/main/kotlin/io/github/franchoy/nudgewhen/ui/ReminderScreen.kt`. The exact-head implementation CI run is `32739280349` (workflow `CI`, event `push`, branch `release/v0.1.2`, head SHA `05503d58416e287afd96cc1fc7c6f78df8fd2784`, conclusion `success`, required `validate` job `success`). Retained Android validation ran `17 / 0 / 0` and `release_gate=NOT_SATISFIED`. Phase 4A planning/audit was accepted with maintainer refinements; Phase 4B `ReminderScreen` was accepted; Phase 4C `MainActivity` integration was accepted; Phase 4D integration audit was accepted with `BLOCKING_DEFECTS NONE`. Source-level integration is `MainActivity -> filesDir -> FileReminderStore -> ReminderController -> ReminderScreen`; `ReminderScreen` owns Compose state; `MainActivity` owns no `MutableState`; production UUID IDs are used; source-level synchronous startup restoration integration is implemented. No `ViewModel`, no `Flow`, no coroutines, no DI, no navigation. No new Android `Activity`, `Service`, `Receiver`, `Provider`, or permission. No new Gradle dependency, test dependency, or validator architecture. Physical-device / end-to-end restart evidence is NOT claimed here. Phase 5 owns device/end-to-end validation; Phase 5 is current / `Planned`; Phase 5 implementation is `NOT_STARTED`. The Phase 4 closure-synchronization commit, push, and exact-head CI are separate maintainer repository actions; this record does not claim that those actions have already occurred.
- **v0.1.2 Phase 5 closure record:** Phase 5 — Integration & Device Validation — is `Complete`. Cumulative Phase 5 experiment evidence is `EXP-0040`. Phase 5B Android artifact identity alignment is landed (commit `6b16a294b3d13151baf23a239e4cf0d330a27d3e`, exact-head CI run `32756426800`, conclusion `success`); the delivered Android identity is `versionCode = 3`, `versionName = "0.1.2"`. Phase 5C exact-head integrated local validation is accepted at HEAD `e6a10bde87aa5841c5669d91512d7040089b100a` (Python validator `Ran 100 tests / OK`; `required` `11/0/0`; `docs` `11/0/0`; full offline `39/0/0`; `release_gate=SATISFIED`; tracked `git diff --check` produced no output; final repository proof clean at HEAD). Phase 5D-R8 one-device runtime acceptance is accepted on one physical `PA2310GBB` running Android 13 (`ONE_PHYSICAL_DEVICE_ONLY`); runtime checkpoints P5-01 through P5-07 are `PASS`; P5-08 is `DEVICE_PROOF_NOT_REQUIRED` because retained deterministic Phase 3 JVM persistence evidence already covers malformed-file rejection. The Phase 5 evidence is explicitly scoped to `PA2310GBB / Android 13` and `ONE_PHYSICAL_DEVICE_ONLY`; it is not a general Android compatibility statement and not a production-readiness statement. No new Android permission, component, Gradle dependency, test dependency, or validator-architecture change was introduced by Phase 5. Phase 5 is marked `Complete`. Phase 6 — Integrated Audit & Agent Evaluation — is now the current lifecycle phase and remains `Planned`; Phase 6 implementation has not started. Phase 7 — Full Pre-Release Gate — remains `Planned`. The v0.1.2 release is **not** claimed ready, merged, tagged, or published by this Phase 5 closure. The Phase 5 closure-synchronization commit, push, and exact-head CI are separate maintainer repository actions; this record does not claim that those actions have already occurred.
...
- **Phase 4 closure record:** Phase 4 is `Complete`. The accepted implementation outcome is `Successful with correction`. The accepted experiment evidence is `EXP-0028`. The implementation/evidence commit is `65a41bf59ef0b05a3ff40217a031a585e503036f` (subject `test: enforce validator repository consistency`, branch `release/v0.1.1`), which directly modified the eight authorized Phase 4 implementation/evidence paths: `.github/workflows/ci.yml`, `docs/agentic-development/experiments/EXP-0028.md` (added), `docs/local-validation.md`, `scripts/validate_local.py`, `tests/__init__.py`, `tests/_helpers.py`, `tests/test_validator_core.py`, and `tests/test_validator_repository.py`. The clean committed-state regression accepted the 64-test matrix (`python3 -B -m unittest tests.test_validator_core tests.test_validator_repository` → `Ran 64 tests / OK`). The clean committed-state validator accepted the full `--require-clean` proof (`./scripts/validate-local.sh --require-clean` → `SUMMARY pass=37 fail=0 skip=0` and `release_gate=SATISFIED`). The exact-head CI run is `31258760694` (workflow `CI`, event `push`, branch `release/v0.1.1`, head SHA `65a41bf59ef0b05a3ff40217a031a585e503036f`, conclusion `success`, required `validate` job `success`). No product functionality, Android behavior, or Android permission was added or changed by the Phase 4 implementation. Historical attempt deviations recorded in `EXP-0028.md` remain historical evidence and are not erased. After Phase 4 closure, Phase 5 became the next current phase. The Phase 4 closure-overlay commit, push, and exact-head CI are separate maintainer repository actions; this record does not claim that those actions have already occurred.
- **Phase 5 closure record:** Phase 5 is `Complete`. The accepted retained technical Phase 5 integration evidence is recorded in `EXP-0029.md`. The retained Phase 5A/5B/5C/5D technical implementations are technically accepted; the retained Phase 5I technical integration is accepted; the retained controlled Gradle bad-distribution checksum rejection proof was directly observed against a temporary `file://` distribution with an isolated `GRADLE_USER_HOME`, a temporary wrapper properties file, and a temporary fake distribution, leaving the repository and its wrapper files untouched. The retained cumulative regression accepted the 78-test matrix (`python3 -B -m unittest tests.test_validator_core tests.test_validator_repository` → `Ran 78 tests / OK`). The retained `required` group ran `11/0/0`, the retained `docs` group ran `11/0/0`, the retained `skip-Android` group ran `22/0/0`, the retained `Android offline` group ran `16/0/0`, and the retained `full offline` run produced `38/0/0` with `release_gate=SATISFIED`. The retained Android metadata is `versionCode = 2` and `versionName = 0.1.1`. The implementation/evidence commit is `edf9da3cf5fef652c595936188e5918c2bd6e7f2` (subject `chore: complete phase 5 implementation and evidence`, branch `release/v0.1.1`) and was successfully pushed to `origin/release/v0.1.1`; that successful push is maintainer-supplied evidence, and this Build did not independently observe it. The exact-head CI run is `31334416352` (workflow `CI`, event `push`, branch `release/v0.1.1`, head SHA `edf9da3cf5fef652c595936188e5918c2bd6e7f2`, conclusion `success`, required `validate` job `success`, debug APK artifact produced); that exact-head CI is maintainer-supplied evidence, and this Build did not independently observe it. No product functionality, Android behavior, or Android permission was introduced by the Phase 5 implementation. Historical attempt deviations and corrections recorded in `EXP-0029.md` remain historical evidence and are not erased. The current phase is Phase 6; Phase 6 remains `Planned`. The Phase 5 closure-overlay repository action is complete: commit `b7f3f5201ab9840be64c383fee33243186fd96ce` (subject `chore: close phase 5 formal closure`, parent `edf9da3cf5fef652c595936188e5918c2bd6e7f2`) was successfully pushed to `origin/release/v0.1.1`, the remote release branch was established at the same full SHA, and exact-head CI run `31468730138` completed successfully with the required `validate` job.
