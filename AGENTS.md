...
- **`nudge-build`** is an intentionally inert primary placeholder. Its deny-all permission boundary means it does not authorize implementation, investigation, repository inspection, mutation, command execution, or consequential work. A later separately authorized increment may replace this placeholder.
...
Raw session exports matching the generic `session-ses_*.md` pattern remain private working material. The readable custom agents (`nudge-plan` and `nudge-audit`) are explicitly denied from reading any path matching `*session-ses_*.md`.

The fresh-session runtime validation of the new harness (config load, default-agent behavior, selectable-agent inventory, and permission probes) requires a fresh OpenCode process and remains separately pending after this content Build. Writing the configuration files does not itself prove that a running OpenCode process loaded them.
...
- **Increment A (machine-readable OpenCode harness):** Increment A introduces a project-local machine-readable OpenCode harness consisting of `opencode.jsonc` and the three custom primary agent definitions under `.opencode/agents/`: `nudge-plan` (default selectable primary, planning-only), `nudge-audit` (read-only auditing primary with a bounded command allowlist), and `nudge-build` (intentionally inert primary placeholder). The built-in selectable agents `build`, `plan`, `general`, `explore`, and `scout` are disabled. Hidden system agents required for compaction, titles, and summaries remain available. The fresh-session runtime validation of the new harness (config load, default-agent behavior, selectable-agent inventory, and permission probes) requires a fresh OpenCode process and remains separately pending. The Increment A content Build writes the configuration files but does not itself prove that a running OpenCode process loaded them.
