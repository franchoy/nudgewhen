...
NudgeWhen is currently in the `v0.1.3` release train, **Editable Local Reminders**, on the single branch `release/v0.1.3`. Phases 0 through 3 are complete; Phase 0 — Release Definition & Bootstrap — is formally closed; Phase 1 — Editing Architecture Contract — is formally closed; Phase 2 — Editing Domain Implementation & JVM Proof — is `Complete`; Phase 3 — Persistence Compatibility Proof — is `Complete`; Phases 4 through 7 remain `Planned`; 4 Complete / 4 Planned. Phase 4 — Minimal Compose Editing UX — is the next lifecycle phase, remains `Planned`, and has not started. The Phase 1 architecture contract is landed and exact-head CI accepted. v0.1.3 Phase 2 has landed the domain-level reminder editing implementation and deterministic JVM proof; editing domain implementation: `LANDED_AND_EXACT_HEAD_CI_ACCEPTED`. v0.1.3 Phase 3 has landed the persistence compatibility proof: the existing NWR1 remains compatible with edit operations; id/index/neighbors/order persist across edit/reload; fresh store/controller restores edited text; existing valid NWR1 supports load/edit/save/reload; Unicode edit round-trip is proven; no production persistence change was made; NWR1 migration: `NOT_REQUIRED_BY_PHASE_3_TEST_EVIDENCE`. Persistence compatibility proof: `LANDED_AND_EXACT_HEAD_CI_ACCEPTED`. Compose editing UX: `NOT_STARTED`. The user-facing editing flow `DOES_NOT_EXIST_YET`. The completed `v0.1.2` — **Local Reminder Foundation** — release remains historical and provides the inherited local reminder baseline. Phases 0 and 1 did not implement reminder editing; Phase 2 implemented only the frozen edit domain API and its deterministic JVM proof and did not implement persistence across edit or any user-facing editing flow; Phase 3 proved persistence compatibility and did not implement any user-facing editing flow. This synchronized lifecycle state does not claim v0.1.3 is merged, tagged, published, or release-ready.
...
Historical `v0.1.2` — **Local Reminder Foundation** — is complete and provides the inherited local reminder baseline. The active `v0.1.3` release is **Editable Local Reminders**. Phases 0 through 3 are complete; Phase 0 — Release Definition & Bootstrap — and Phase 1 — Editing Architecture Contract — are formally closed; Phase 2 — Editing Domain Implementation & JVM Proof — is `Complete`; Phase 3 — Persistence Compatibility Proof — is `Complete`; Phases 4 through 7 remain `Planned`; 4 Complete / 4 Planned. Phase 4 — Minimal Compose Editing UX — is the next lifecycle phase, remains `Planned`, and has not started. Reminder editing was not implemented by Phases 0 or 1; Phase 2 implemented only the frozen edit domain API and its deterministic JVM proof; Phase 3 proved persistence compatibility and did not implement any user-facing editing flow. This synchronized lifecycle state does not claim v0.1.3 is merged, tagged, published, or release-ready.
