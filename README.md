...
NudgeWhen is currently in the `v0.1.3` release train, **Editable Local Reminders**, on the single branch `release/v0.1.3`. Phases 0 through 6 are complete; Phase 0 — Release Definition & Bootstrap — is formally closed; Phase 1 — Editing Architecture Contract — is formally closed; Phase 2 — Editing Domain Implementation & JVM Proof — is formally closed; Phase 3 — Persistence Compatibility Proof — is formally closed; Phase 4 — Minimal Compose Editing UX — is `Complete`; Phase 5 — Integration & Device Validation — is `Complete`; Phase 6 — Integrated Audit & Reconciliation — is `Complete`; phase model: `7 Complete / 1 Planned`. Phase 6-C real-use nudge-land evidence is `Complete` at landing-core commit `844db073394b9d76e847f4078b7eb12b79b07a78` (terminal state `LANDED / S10_LANDED`); the completed single-use Phase 6-C authorization `nudgewhen-v0.1.3-phase6c-first-use-001` is `PROHIBITED` from reuse. Phase-6 formal closure synchronization is `Complete`. Phase 7 — Full Pre-Release Gate — is the next separately authorized lifecycle gate, currently `Planned` and `NOT_STARTED`. The Phase 1 architecture contract is landed and exact-head CI accepted. v0.1.3 Phase 2 has landed the domain-level reminder editing implementation and deterministic JVM proof; editing domain implementation: `LANDED_AND_EXACT_HEAD_CI_ACCEPTED`. v0.1.3 Phase 3 has landed the persistence compatibility proof: the existing NWR1 remains compatible with edit operations; id/index/neighbors/order persist across edit/reload; fresh store/controller restores edited text; existing valid NWR1 supports load/edit/save/reload; Unicode edit round-trip is proven; no production persistence change was made; NWR1 migration: `NOT_REQUIRED_BY_PHASE_3_TEST_EVIDENCE`. Persistence compatibility proof: `LANDED_AND_EXACT_HEAD_CI_ACCEPTED`. v0.1.3 Phase 4 has landed the minimal Compose editing UX layered onto the existing `ReminderScreen` integration; existing reminder text can now be edited from `ReminderScreen` while preserving identity, list position, and persistence contracts inherited from Phases 2/3. Compose editing UX: `LANDED_AND_EXACT_HEAD_CI_ACCEPTED`. User-facing textual editing: `IMPLEMENTED_AT_SOURCE_LEVEL`. Integrated local validation for v0.1.3 Phase 5: `PASS` (Python validator `Ran 100 tests / OK`; `required` `11/0/0`; `docs` `11/0/0`; full offline `39 / 0 / 0`; raw validator literal: `release_gate=SATISFIED`); current-facing lifecycle release-gate classification: `SATISFIED_AT_PHASE_5_VALIDATION_HEAD`; bounded physical-device validation: `PASS_ONE_PHYSICAL_DEVICE_ONLY`; P5_01 through P5_13: `PASS`. Current Android identity: `versionCode 4 / versionName 0.1.3`. Target Android identity: `versionCode 4 / versionName 0.1.3`. Release readiness: `NO`. Merged: `NO`. Tagged: `NO`. Published: `NO`. General Android compatibility: `NOT_CLAIMED`. Multi-device validation: `NOT_CLAIMED`. Production readiness: `NOT_CLAIMED`. The completed `v0.1.2` — **Local Reminder Foundation** — release remains historical and provides the inherited local reminder baseline. Phases 0 and 1 did not implement reminder editing; Phase 2 implemented only the frozen edit domain API and its deterministic JVM proof and did not implement persistence across edit or any user-facing editing flow; Phase 3 proved persistence compatibility and did not implement any user-facing editing flow; Phase 4 implemented the bounded Compose editing UX on `ReminderScreen` and did not perform integrated/device validation or Android identity alignment. Phase 5 — Integration & Device Validation — has landed the Android artifact identity alignment, the integrated local validation at HEAD `1cfb9c373abfa24cf10f23daa152f4a410932d26`, and the bounded one-physical-device runtime acceptance; integrated/device validation and Android identity alignment are `Complete`; general Android compatibility, multi-device validation, and production readiness remain `NOT_CLAIMED`. This synchronized lifecycle state does not claim v0.1.3 is merged, tagged, published, or release-ready.
...
These are long-term goals. Voice-first capture and contextual-reminder behavior remain future goals. The completed `v0.1.2` Local Reminder Foundation supplies the inherited minimal local reminder lifecycle and accepted one-device Android 13 runtime evidence. Phase 0 — Release Definition & Bootstrap — and Phase 1 — Editing Architecture Contract — of the active `v0.1.3` release are formally closed; Phase 2 — Editing Domain Implementation & JVM Proof — is `Complete`; Phase 3 — Persistence Compatibility Proof — is `Complete`; Phase 4 — Minimal Compose Editing UX — is `Complete`; Phase 5 — Integration & Device Validation — is `Complete`; Phase 6 — Integrated Audit & Reconciliation — is `Complete`; phase model: `7 Complete / 1 Planned`. Phase 6-C real-use nudge-land evidence is `Complete` at landing-core commit `844db073394b9d76e847f4078b7eb12b79b07a78` (terminal state `LANDED / S10_LANDED`); the completed single-use Phase 6-C authorization `nudgewhen-v0.1.3-phase6c-first-use-001` is `PROHIBITED` from reuse. Phase 7 — Full Pre-Release Gate — is `Planned` and `NOT_STARTED`. Phases 0 and 1 did not implement reminder editing; Phase 2 implemented only the frozen edit domain API and its deterministic JVM proof and did not implement persistence across edit or any user-facing editing flow; Phase 3 proved persistence compatibility and did not implement any user-facing editing flow; Phase 4 implemented the bounded Compose editing UX on `ReminderScreen` and did not perform integrated/device validation or Android identity alignment. Current-facing lifecycle release-gate classification: `SATISFIED_AT_PHASE_5_VALIDATION_HEAD`. Merged: `NO`. Tagged: `NO`. Published: `NO`. Release readiness: `NO`. General Android compatibility: `NOT_CLAIMED`. Multi-device validation: `NOT_CLAIMED`. Production readiness: `NOT_CLAIMED`.
...
Historical `v0.1.2` — **Local Reminder Foundation** — is complete and provides the inherited local reminder baseline. The active `v0.1.3` release is **Editable Local Reminders**. Phases 0 through 6 are complete; Phase 0 — Release Definition & Bootstrap — and Phase 1 — Editing Architecture Contract — are formally closed; Phase 2 — Editing Domain Implementation & JVM Proof — is `Complete`; Phase 3 — Persistence Compatibility Proof — is `Complete`; Phase 4 — Minimal Compose Editing UX — is `Complete`; Phase 5 — Integration & Device Validation — is `Complete`; Phase 6 — Integrated Audit & Reconciliation — is `Complete`; phase model: `7 Complete / 1 Planned`. Phase 6-C real-use nudge-land evidence is `Complete` at landing-core commit `844db073394b9d76e847f4078b7eb12b79b07a78` (terminal state `LANDED / S10_LANDED`); the completed single-use Phase 6-C authorization `nudgewhen-v0.1.3-phase6c-first-use-001` is `PROHIBITED` from reuse. Phase 7 — Full Pre-Release Gate — is `Planned` and `NOT_STARTED`. Reminder editing was not implemented by Phases 0 or 1; Phase 2 implemented only the frozen edit domain API and its deterministic JVM proof; Phase 3 proved persistence compatibility and did not implement any user-facing editing flow; Phase 4 implemented the bounded Compose editing UX on `ReminderScreen` and did not perform integrated/device validation or Android identity alignment. Phase 5 — Integration & Device Validation — has landed the Android artifact identity alignment, the integrated local validation at HEAD `1cfb9c373abfa24cf10f23daa152f4a410932d26`, and the bounded one-physical-device runtime acceptance; integrated/device validation and Android identity alignment are `Complete`; general Android compatibility, multi-device validation, and production readiness remain `NOT_CLAIMED`. Current-facing lifecycle release-gate classification: `SATISFIED_AT_PHASE_5_VALIDATION_HEAD`. Release readiness: `NO`. Merged: `NO`. Tagged: `NO`. Published: `NO`. This synchronized lifecycle state does not claim v0.1.3 is merged, tagged, published, or release-ready.
