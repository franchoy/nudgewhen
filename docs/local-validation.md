# Local Validation — NudgeWhen v0.1.4

**Document status:** Accepted — reusable validator architecture carried forward; v0.1.4 — Mark Reminder Done — is released and closed on `release/v0.1.4`; v0.1.4 has been merged into `main`, tagged with the annotated `v0.1.4` tag, and published as a GitHub Release; v0.1.4 is the latest stable published release; the post-v0.1.4 maintenance window is formally closed; v0.1.5 has not started; the retained current release identity remains v0.1.4 / `release/v0.1.4` until separately authorized v0.1.5 Phase 0; Phase 0 — Release Definition & Bootstrap — is `Complete`; Phase 1 — Done State Architecture Contract — is `Complete`; Phase 2 — Done State Domain Implementation & JVM Proof — is `Complete`; Phase 3 — Persistence Compatibility / Migration Proof — is `Complete`; Phase 4 — Minimal Compose Integration — is `Complete`; Phase 5 — Integration & Device Validation — is `Complete`; Phase 6 — Integrated Audit & Reconciliation — is `Complete`; Phase 7 — Full Pre-Release Gate — is `Complete` and formally complete following the accepted Phase-7-D independent candidate audit, Phase-7-E native nudge-land landing, and Phase-7-F terminal maintainer acceptance; phase model: `8 Complete / 0 Planned`; no lifecycle phase remains inside the accepted eight-phase v0.1.4 model; historical v0.1.0 / v0.1.1 / v0.1.2 / v0.1.3 release evidence remains historical evidence.
...
The phase-list path comes from `release_documents.phase_list`. The phase range comes from `phase_model.first_phase` and `phase_model.last_phase`. Expected statuses come from `phase_model.expected_statuses`. The `docs/phase-headings` check requires the exact ordered and unique declared range; for the current contract, Phase 0 through Phase 7 are required. Each phase is bounded from its own phase heading to the next phase heading or EOF. Each bounded phase requires exactly one `### Status` heading. The status subsection ends at the next Markdown heading of any level or the phase-section end. Exactly one nonblank, unformatted `Complete` or `Planned` line is required. The observed value must equal the phase-specific contract value; a later heading or phase cannot supply an earlier phase's status. The successful state for the current contract is `8 Complete, 0 Planned`.

The `docs/readme-active-release` check requires the README to contain the contract's active release version and active branch. In the historical Phase 3A3a implementation, the `docs/charter-consistency` check used the charter path from `release_documents.charter` and enforced the then-current absence-of-product-functionality rule. The current implementation has since evolved that same check to apply the active contract's `product_scope.allowed_capabilities` authorization: a positive, non-negated claim for a machine-recognized capability that is not authorized by the contract fails; an authorized positive claim is permitted but is not required; negated and explicit non-goal occurrences remain permitted; and the charter must still contain an explicit `Non-goals`, `Explicit non-goals`, or `Out of scope` section. Inline-negation context retains the historical approximately ±300-character window but is now clamped to the containing Markdown `##` section so that negation in a neighbouring section cannot mask a positive claim. The historical v0.1.0 charter and phase list are no longer treated as active sources for the active release-document checks. Historical `EXP-0007` structure validation is preserved as an intentionally historical stable check that is independent of the active contract.
...
At the time Phase 3A5 was accepted, the successful repository expected version code `1` and version name `0.1.0` for the `apk-metadata` check because those were then the contract's current committed `current_version_code` and `current_version_name` values. Phase 5D subsequently updated the candidate contract and Gradle metadata to version code `2` and version name `0.1.1` without requiring another Python source edit for those APK fields; those were the committed v0.1.1 metadata. The v0.1.2 release subsequently introduced `current_version_code = 3` and `current_version_name = 0.1.2`; those were the v0.1.2 metadata. The v0.1.3 release subsequently introduced `current_version_code = 4` and `current_version_name = 0.1.3`; those were the v0.1.3 committed metadata. The v0.1.4 release subsequently introduced `current_version_code = 5` and `current_version_name = 0.1.4`; those are the current committed v0.1.4 metadata. The validator continues to validate APK metadata against `current_version_code` and `current_version_name` rather than against the target fields.
...
#### JVM-test integration

The current Android validation sequence executes the JVM unit-test integration check (`android/jvm-tests`) between Gradle project discovery and the debug build:
...
The `android/jvm-tests` check runs the separate Gradle task `:app:testDebugUnitTest` from the Gradle build of the `:app` module, using `--offline` when the runner is started in offline mode. A JVM-test failure short-circuits the later `gradle-build`, `apk-exists`, `apk-metadata`, and `merged-manifest` checks for the current run, so that an obvious JVM-test regression does not silently pass alongside a later artifact-validation regression. The check is part of the `android` group's content sequence; it is not exposed as a separate top-level validation group, and it is not a `release_gate_requires_groups` entry on its own. The `android` group therefore continues to be one group with seventeen passes, and a complete `required` + `docs` + `android` run still satisfies the release gate.
...
| `--group android --offline` (succeeding) | `SUMMARY pass=17 fail=0 skip=0` | `NOT_SATISFIED` | `0` |
| `--skip-android` (succeeding) | `SUMMARY pass=22 fail=0 skip=0` | `NOT_SATISFIED` | `0` |
| `--offline` all-groups (succeeding) | `SUMMARY pass=39 fail=0 skip=0` | `SATISFIED` | `0` |
| `--require-clean --offline` all-groups (succeeding) | `SUMMARY pass=41 fail=0 skip=0` | `SATISFIED` | `0` |
...
The current phase model is `8 Complete, 0 Planned`: `Complete` is `Phase 0`, `Phase 1`, `Phase 2`, `Phase 3`, `Phase 4`, `Phase 5`, `Phase 6`, and `Phase 7`; no phase is `Planned`. Phase 0 — `Release Definition & Bootstrap` — is `Complete`; Phase 1 — `Done State Architecture Contract` — is `Complete`; Phase 2 — `Done State Domain Implementation & JVM Proof` — is `Complete`; Phase 3 — `Persistence Compatibility / Migration Proof` — is `Complete`; Phase 4 — `Minimal Compose Integration` — is `Complete`; Phase 5 — `Integration & Device Validation` — is `Complete`; Phase 6 — `Integrated Audit & Reconciliation` — is `Complete`; Phase 7 — `Full Pre-Release Gate` — is `Complete` and formally complete. No lifecycle phase remains inside the accepted eight-phase v0.1.4 model. Phase-5 implementation is `LANDED_AND_EXACT_HEAD_CI_ACCEPTED`. Historical v0.1.3 evidence recorded in this section — the historical `8 Complete / 0 Planned` v0.1.3 phase model, the formal Phase 0-C6 closure-candidate validation, the Phase 6 — `Integrated Audit & Reconciliation` — `Complete` in the v0.1.3 closure-synchronization candidate, the Phase 6-C `Complete` landing-core commit `844db073394b9d76e847f4078b7eb12b79b07a78`, the Phase 7 — `Full Pre-Release Gate` — `Complete` in the v0.1.3 closure-synchronization candidate, the v0.1.3 formal Phase 6 closure, and the terminated v0.1.3 bootstrap exception — remains historical evidence for the v0.1.3 release train and does not describe the active `v0.1.4` state. The required-group total of eleven passes consists of one `release-contract` check (loaded, structurally validated, and cross-checked by Phase 3A2), seven pre-Phase-5 checks (`files`, `no-prohibited`, `gradlew-exec`, `shell-exec`, `wrapper-jar`, `gitignore`, `gitattributes`), and three Phase 5 additions (`wrapper-jar-sha256`, `gitignore-python`, `dependabot-yaml`); `1 + 7 + 3 = 11`. The Phase 5A `wrapper-jar-sha256` check confirms the committed `gradle-wrapper.jar` matches the approved `WRAPPER_JAR_EXPECTED_SHA`; the Phase 5B `gitignore-python` check confirms the canonical Python bytecode ignore rules (`__pycache__/`, `*.py[cod]`) are present; the Phase 5C `dependabot-yaml` check confirms the bounded Dependabot configuration. The Android-group total of seventeen passes is the sum of the six prerequisite passes (`python`, `java`, `sdk-platform`, `sdk-build-tools`, `aapt2`, `gradlew-exec`) and the eleven content checks (`build-config`, `version-catalog`, `gradle-wrapper`, `app-build-config`, `source-manifest`, `gradle-projects`, `jvm-tests`, `gradle-build`, `apk-exists`, `apk-metadata`, `merged-manifest`). The ordinary all-groups total of thirty-nine passes is the sum of eleven required, eleven docs, and seventeen android (`11 + 11 + 17 = 39`). The skip-Android total of twenty-two passes is the sum of eleven required and eleven docs (`11 + 11 = 22`). The docs-group total of eleven passes is the sum of `utf8`, `trailing-ws`, `gradlew-bat-crlf`, `md-links`, `phase-headings`, `phase-status`, `readme-active-release`, `charter-consistency`, `exp7-structure`, `no-pii`, and the Phase 4 repository-consistency check (`repository-consistency`); for the historical `v0.1.2` release contract, the active phase list reported `7 Complete, 1 Planned` (Phase 0, Phase 1, Phase 2, Phase 3, Phase 4, Phase 5, and Phase 6 were `Complete`; Phase 7 was `Planned`).
...
The current standard Python regression inventory is `245 tests`: `101 validator regression tests + 144 deterministic nudge-land tests = 245`. The canonical command is `python3 -B -m unittest tests.test_validator_core tests.test_validator_repository tests.test_nudge_land_cli tests.test_nudge_land_ledger tests.test_nudge_land_runtime`. Historical validation commands elsewhere in this document remain historical evidence and are not rewritten merely because the current standard regression expands.
...
The `android` group then runs `aapt2 dump badging` on the produced debug APK and requires the exact package, version code, version name, compile SDK, minimum SDK, target SDK, and launcher activity values generated from the validated contract's `android` block (`package_name`, `current_version_code`, `current_version_name`, `compile_sdk`, `min_sdk`, `target_sdk`, and `launcher_activity_merged`). Phase 5D introduced the candidate `current_version_code = 2` and `current_version_name = 0.1.1` (v0.1.1 metadata); the v0.1.2 release subsequently introduced `current_version_code = 3` and `current_version_name = 0.1.2` (v0.1.2 metadata); the v0.1.3 release subsequently introduced `current_version_code = 4` and `current_version_name = 0.1.3` (v0.1.3 metadata); the v0.1.4 release subsequently introduced `current_version_code = 5` and `current_version_name = 0.1.4` (current v0.1.4 metadata). The APK-metadata check therefore currently expects version code `5` and version name `0.1.4`.

## Release-charter product-scope predicate

The `docs` group evaluates the active release charter against the contract's `product_scope.allowed_capabilities` list. The closed machine-recognized capability identifiers are:
...
For each identifier that is not present in `allowed_capabilities`, a positive, non-negated charter claim is a validation defect. If an identifier is authorized, a positive claim is permitted but is not required. Negated and explicit non-goal occurrences remain permitted regardless of authorization. The charter must still contain an explicit `Non-goals`, `Explicit non-goals`, or `Out of scope` section. Inline-negation matching retains the historical approximately ±300-character context but clamps that context to the containing Markdown `##` section.

The current active `v0.1.4` contract uses `allowed_capabilities = ["reminders", "persistence"]`; therefore only those two capabilities are positively authorized by the current release. This predicate is release-neutral: future releases can authorize an explicit subset through the contract without hard-coding a release version in the validator.
...
The workflow path is `.github/workflows/ci.yml`; its `validate` job runs the combined standard Python regression suite (the existing validator regression modules plus the deterministic nudge-land modules) immediately before the clean-checkout validator gate. The historical clean committed/CI Phase 4 validation sequence was:
...
Phase 5 is `Complete`. Phases 0 through 7 are `Complete`; there is no active release phase; release-PR opening is the next separately authorized repository action. The committed v0.1.1 Android metadata is `versionCode = 2` and `versionName = 0.1.1`. Those values are committed in the maintainer-supplied Phase 5 implementation/evidence commit `edf9da3cf5fef652c595936188e5918c2bd6e7f2`. The implementation/evidence commit and its successful push and exact-head CI are established by maintainer-supplied repository evidence; this recovery Build does not independently query the remote.
...
v0.1.1 Android metadata:
versionCode = 2
versionName = 0.1.1

historical implementation/evidence commit:
edf9da3cf5fef652c595936188e5918c2bd6e7f2

historical lifecycle:
Phases 0 through 7 Complete
no active v0.1.1 release phase remained
...
Phases 0 through 7 are `Complete`; there is no active release phase; release-PR opening is the next separately authorized repository action.

## Product functionality

The suite does not introduce reminder, notification, voice, location, persistence, networking, contextual, or background functionality. It validates the technical baseline only. For the current active `v0.1.4` contract, `allowed_capabilities = ["reminders", "persistence"]`, and the release charter remains consistent with that authorization state. The Phase-2 done-state DOMAIN behavior is implemented, landed, and exact-head-CI accepted; the Phase-3 persistence compatibility / migration implementation is implemented, landed, and exact-head-CI accepted; the Phase-4 bounded source-level done-state Compose integration is implemented, landed, and exact-head-CI accepted; the Phase-5 integrated/device validation and Android identity alignment are implemented, landed, and exact-head-CI accepted.
