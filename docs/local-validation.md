...
The cumulative regression inventory is `98 tests` from `python3 -B -m unittest tests.test_validator_core tests.test_validator_repository`.
...
The current active `v0.1.1` contract uses `allowed_capabilities = []`; therefore none of the seven machine-recognized capabilities is positively authorized by the current release. This predicate is release-neutral: future releases can authorize an explicit subset through the contract without hard-coding a release version in the validator.
...
The suite does not introduce reminder, notification, voice, location, persistence, networking, contextual, or background functionality. It validates the technical baseline only. For the current active `v0.1.1` contract, `allowed_capabilities = []`, and the release charter remains consistent with that no-product-capability authorization state.
