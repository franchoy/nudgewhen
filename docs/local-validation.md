...
**Document status:** Accepted — reusable validator architecture carried forward; active `v0.1.4` release train on `release/v0.1.4`; Phase 0 — Release Definition & Bootstrap — is `Complete`; Phase 1 — Done State Architecture Contract — is `Complete`; Phase 2 — Done State Domain Implementation & JVM Proof — is `Complete`; Phase 3 — Persistence Compatibility / Migration Proof — is `Complete`; Phase 4 — Minimal Compose Integration — is `Complete`; Phase 5 — Integration & Device Validation — is `Complete`; Phase 6 — Integrated Audit & Reconciliation — is `Complete`; Phase 7 — Full Pre-Release Gate — is `Complete` and formally complete following the accepted Phase-7-D independent candidate audit, Phase-7-E native nudge-land landing, and Phase-7-F terminal maintainer acceptance; phase model: `8 Complete / 0 Planned`; no lifecycle phase remains inside the accepted eight-phase v0.1.4 model; v0.1.4 is **not** merged, **not** tagged, **not** published, and is not claimed release-ready; pre-v0.1.4 maintenance window is closed; latest stable published release is `v0.1.3`; historical v0.1.0 / v0.1.1 / v0.1.2 / v0.1.3 release evidence remains historical evidence.
...
The current standard Python regression inventory is `244 tests`: the existing 100 validator regression tests plus the accepted 144 deterministic nudge-land tests. The canonical command is `python3 -B -m unittest tests.test_validator_core tests.test_validator_repository tests.test_nudge_land_cli tests.test_nudge_land_ledger tests.test_nudge_land_runtime`. Historical validation commands elsewhere in this document remain historical evidence and are not rewritten merely because the current standard regression expands.
