...
The suite does not introduce reminder, notification, voice, location, persistence, networking, contextual, or background functionality. It validates the technical baseline only. For the current active `v0.1.4` contract, `allowed_capabilities = ["reminders", "persistence"]`, and the release charter remains consistent with that authorization state. Done-state functionality is **not** yet implemented; the active `v0.1.4` release remains at Phase 0 `Complete`, Phase 1 `Complete` (architecture-only), and Phase 2 `Planned` / not started.
