...
At the time Phase 3A5 was accepted, the successful repository expected version code `1` and version name `0.1.0` for the `apk-metadata` check because those were then the contract's current committed `current_version_code` and `current_version_name` values. Phase 5D subsequently updated the candidate contract and Gradle metadata to version code `2` and version name `0.1.1` without requiring another Python source edit for those APK fields; those were the committed v0.1.1 metadata. The v0.1.2 release subsequently introduced `current_version_code = 3` and `current_version_name = 0.1.2`; those are the current committed metadata. The validator continues to validate APK metadata against `current_version_code` and `current_version_name` rather than against the target fields.
...
The cumulative regression inventory is `100 tests` from `python3 -B -m unittest tests.test_validator_core tests.test_validator_repository`.
...
The `android` group then runs `aapt2 dump badging` on the produced debug APK and requires the exact package, version code, version name, compile SDK, minimum SDK, target SDK, and launcher activity values generated from the validated contract's `android` block (`package_name`, `current_version_code`, `current_version_name`, `compile_sdk`, `min_sdk`, `target_sdk`, and `launcher_activity_merged`). Phase 5D introduced the candidate `current_version_code = 2` and `current_version_name = 0.1.1` (v0.1.1 metadata); the v0.1.2 release subsequently introduced `current_version_code = 3` and `current_version_name = 0.1.2` (current metadata). The APK-metadata check therefore currently expects version code `3` and version name `0.1.2`.
...
The current active `v0.1.2` contract uses `allowed_capabilities = ["reminders", "persistence"]`; therefore only those two capabilities are positively authorized by the current release. This predicate is release-neutral: future releases can authorize an explicit subset through the contract without hard-coding a release version in the validator.
...
The suite does not introduce reminder, notification, voice, location, persistence, networking, contextual, or background functionality. It validates the technical baseline only. For the current active `v0.1.2` contract, `allowed_capabilities = ["reminders", "persistence"]`, and the release charter remains consistent with that authorization state.
