# Local Validation — NudgeWhen v0.1.3

**Document status:** Release-aware carried-forward baseline — active `v0.1.3` release train on `release/v0.1.3`; Phases 0 through 7 complete; Phase 0 — Release Definition & Bootstrap — is formally closed following accepted Phase 0-C6 closure-candidate validation; 8 Complete / 0 Planned; Phase 6 — Integrated Audit & Reconciliation — is `Complete`; Phase 6-C real-use nudge-land evidence is `Complete` at landing-core commit `844db073394b9d76e847f4078b7eb12b79b07a78` (terminal state `LANDED / S10_LANDED`); Phase 7 — Full Pre-Release Gate — is `Complete` in this closure-synchronization candidate; the narrow v0.1.3 bootstrap exception is terminated; the reusable validator architecture and current validation inventory are carried forward unchanged.
...
The current active `v0.1.3` contract uses `allowed_capabilities = ["reminders", "persistence"]`; therefore only those two capabilities are positively authorized by the current release. This predicate is release-neutral: future releases can authorize an explicit subset through the contract without hard-coding a release version in the validator.
...
The suite does not introduce reminder, notification, voice, location, persistence, networking, contextual, or background functionality. It validates the technical baseline only. For the current active `v0.1.3` contract, `allowed_capabilities = ["reminders", "persistence"]`, and the release charter remains consistent with that authorization state.
