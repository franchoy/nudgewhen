...
Direct reads of `.git/HEAD`, loose reference files, packed references, the Git index, or other `.git` internals are not substitutes for an explicitly required Git command. When a mandatory Git command is unavailable or denied, the agent stops and reports the unavailable evidence rather than reconstructing partial state from Git internals, a wrapper, a pipeline, redirection, command substitution, or an ad hoc script.

## Hard-stop precedence over task completion

A hard stop is a required FAIL, BLOCKED, or unexpected command failure, a baseline mismatch, a missing required tracked file, an unavailable mandatory command or tool capability, an unavailable mandatory command exit status, or a detected scope deviation. A desired final deliverable never overrides a hard stop. The agent stops further consequential work immediately, reports neutrally, and preserves the failure or deviation in the experiment record. It must not continue the requested analysis after substituting partial evidence. A standalone Python invocation, an ad hoc shell check, a direct read of `.git` internals, or a wrapper that hides the primary command's exit status does not satisfy a hard-stop requirement.
...
Every phase, subphase, and Build-stage report must end with a validation checklist that uses exactly one of `[x] PASS`, `[ ] FAIL`, `[ ] BLOCKED`, or `[-] NOT APPLICABLE` per item, with concise directly observed evidence. A blanket "all checks passed" statement is insufficient. The four-state checklist and the standard report format are the generic default.

An explicit task specification may replace REPORT FORMAT only when the override is explicit and unambiguous. A report-format override does **not** waive or override:
...
A task-specific format override is a FORMAT exception only. It does not authorize scope expansion, evidence fabrication, or any other substantive deviation. `AGENTS.md` and the experiment protocol remain authoritative over any task-specific format override. The full contract is in `AGENTS.md` §"Mandatory phase-checklist reporting" and in the experiment protocol.
...
- **`nudge-plan`** — the default selectable primary agent. Planning-only. Its bounded command allowlist permits only the local read-only Git inspection commands and the `opencode --version` query explicitly listed in its definition; all other shell commands are denied. File-editing tools are not exposed. It does not mutate repository content, the Git index, commits, branches, remotes, configuration, dependencies, or external systems. Machine-readable permissions remain capability ceilings: every command and read still requires the exact current maintainer authorization.
...
The global deny baseline in `opencode.jsonc` (`"permission": {"*": "deny"}`) is overridden only by explicit custom-agent permission rules defined inside each agent's `permission` block. The JSON and agent files do not independently grant permission; the global deny baseline applies to every operation not explicitly allowed by a custom-agent rule. For `nudge-plan`, file edits, staging, commits, branch or remote mutation, network commands, shell wrappers, unlisted shell commands, dependency tools, external-system actions, and direct substitutes for mandatory Git commands remain denied.

Fresh-process runtime validation is mandatory after every permanent harness modification. The accepted `nudge-build` boundary is recorded in `EXP-0017`, including separate automatic-allow, one-shot approval, and representative denial evidence. A permanent `nudge-plan` permission change likewise requires fresh-process evidence that every allowlisted local Git command and `opencode --version` execute automatically, while representative staging, network, shell-wrapper, unlisted-command, and file-edit attempts remain denied. Ordinary implementation use remains subject to exact current maintainer authorization, and every future permanent harness modification requires fresh-process revalidation. Writing configuration content does not itself prove that a running OpenCode process loaded it.
